...
program
  .requiredOption('-k, --key-type <type>', 'Key type: rsa-2048, rsa-4096, or ed25519', (value) => {
    const validKeyTypes = ['rsa-2048', 'rsa-4096', 'ed25519']
    if (!validKeyTypes.includes(value)) {
      throw new Error(`Invalid key type '${value}'. Allowed values are: ${validKeyTypes.join(', ')}`)
    }
    return value
  })
  .option('-p, --passphrase <pass>', 'Passphrase to encrypt the private key (default: none)')
  .option(
    '-n, --num-keys <number>',
