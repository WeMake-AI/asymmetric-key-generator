...
const savePairs = async () => {
  ...
  if (count > 1) {
    ...
    result = await window.utils.savePairs([{ privateKey, publicKey }], keyType, { count: 1, passphrase })
  }
...
function updateButtonLabel() {
  ...
  savePairButton.textContent = isValid ? `SAVE ${count} PAIR${count > 1 ? 'S' : ''}` : 'SAVE PAIR'
  generateKeysButton.textContent = isValid ? `Generate ${count} Key Pair${count > 1 ? 's' : ''}` : 'Generate Key Pair'
}
